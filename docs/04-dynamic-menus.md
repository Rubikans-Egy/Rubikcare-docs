...
## ⚠️ ملاحظة معمارية حرجة: استقرار الاتجاه والتنقل

نظراً للحساسية العالية في تزامن واجهة المستخدم مع حالة اللغة والاتجاه (RTL/LTR)، يجب الالتزام بالقواعد التالية عند التعامل مع القائمة الجانبية أو التنقل:

1. **ممنوع استخدام `forceLoad: true`**: 
   عند التنقل عبر عناصر القائمة أو تبديل الأوضاع، يجب استخدام `Navigation.NavigateTo(url)` فقط. استخدام `forceLoad: true` يدمر حالة الجلسة (Session State) ويسبب وميضاً بصرياً وفقداناً مؤقتاً لاتجاه الصفحة.
   
2. **آلية حفظ الاتجاه**: 
   يعتمد استقرار اتجاه القائمة الجانبية بشكل حاسم على وجود سمة `dir` ديناميكية في `MainLayout.razor`. أي تعديل على هيكل `MainLayout` يجب أن يحافظ على: 
   `<div class="main-wrapper" dir="@(TranslationState.CurrentLanguage == "ar" ? "rtl" : "ltr")">`

3. **للحصول على تفاصيل تقنية كاملة** حول سبب هذه القيود وكيفية اختبار استقرار الواجهة، يرجى مراجعة الوثيقة المخصصة: 
