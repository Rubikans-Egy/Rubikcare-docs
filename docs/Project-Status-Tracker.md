...
---
## 8. حالة المشروع الحالية والمهام المعلقة (Current Status & Action Items)

**آخر تحديث:** 6 سبتمبر 2026  
**الهدف:** تتبع واضح للمهام المنجزة والمعلقة لضمان عدم ضياع أي متطلبات.

### ✅ ما تم إنجازه حديثاً (Completed)
| # | المهمة | التفاصيل |
|---|--------|----------|
| 1 | **إصلاح توجيه "Next Refill"** | تم إصلاح المشكلة حيث كان الضغط على المعاد القادم يوجه المريض لصفحة برامج الطبيب. الآن يتم التوجيه بشكل صحيح إلى صفحة "تفاصيل البرنامج" الخاصة بالمريض. |

---

### 🔴 مهام معلقة وبحاجة إلى حل (Pending Tasks / Bugs)

| # | الميزة / الصفحة | وصف المشكلة أو المطلوب | الأولوية |
|---|-----------------|------------------------|:---:|
| 1 | **صفحة التراخيص المعلقة**<br>`/admin/pending-licenses` | ظهور خطأ **404 Not Found** (عذراً، الصفحة التي تبحث عنها غير موجودة أو تم نقلها). | 🔴 عالية |
| 2 | **صفحة المحادثات** | ظهور خطأ **404 Not Found** عند محاولة الدخول للمحادثات. | 🔴 عالية |
| 3 | **حالة الطلب (قاعدة البيانات)** | حالة طلب المريض تظهر كعلامات استفهام (`??? ?????`) عند العرض باللغة العربية. <br>*(المطلوب: ضبط الـ Collation أو الـ Encoding في الداتا بيز لدعم العربية بشكل صحيح).* | 🔴 عالية |
| 4 | **أقرب الصيدليات** | الميزة لا تعمل بشكل صحيح؛ تقوم بإرجاع "صيدلية روبيك كير" فقط بدلاً من البحث الفعلي عن الصيدليات القريبة جغرافيا. | 🟡 متوسطة |
| 5 | **دعواتي (وضع العيادة)** | 1. تحتاج إلى تظبيط في الـ CSS.<br>2. المنطق (Logic): يجب أن تتحدث الحالة تلقائياً لتصبح "مفعلة" عندما يقبل المريض الدعوة وينضم للبرنامج بنجاح. | 🟡 متوسطة |
| 6 | **تعدد شركات الدواء** | تجربة المستخدم (UX) تكون "مُربكة" (Clunky) وغير سلسة عند محاولة الإدارة أو التنقل بين شركتي دواء مختلفتين تحت نفس الحساب. | 🟡 متوسطة |

---

### 📋 خطة العمل المقترحة (Next Steps)
1. **فورياً:** فحص الـ Routing في مشروع الـ Web/PWA لحل مشكلتي الـ 404 (التراخيص المعلقة والمحادثات).
2. **قاعدة البيانات:** تشغيل استعلام لتحديث الـ Collation للجداول المعنية بحالة الطلبات لضمان دعم اللغة العربية.
3. **المنطق البرمجي:** مراجعة خدمة "أقرب الصيدليات" (GPS Logic) ومراجعة تحديث حالة الدعوات في الـ API.
4. **تحسين الـ UX:** مراجعة واجهة التنقل بين شركات الدواء المتعددة لجعلها أكثر سلاسة.
